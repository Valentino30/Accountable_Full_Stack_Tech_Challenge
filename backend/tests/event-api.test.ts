...
describe("Events API", () => {
  ...
  test("GET /api/events filters by country, date, and team", async () => {
    ...
    const resCountry = await request(app).get("/api/events").query({ country: "Aland" }).expect(200);
    expect(resCountry.body[0].country).toBe("Aland");
    expect(resCountry.body[0].homeTeam).toBe("Alpha");

    const resDate = await request(app).get("/api/events").query({ date: "2030-01-01" }).expect(200);
    expect(resDate.body[0].homeTeam).toBe("Alpha");

    const resTeam = await request(app).get("/api/events").query({ team: "Beta" }).expect(200);
